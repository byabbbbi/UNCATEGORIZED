...
현재 게임 본체는 상태 저장과 판정을 모두 브라우저에서 처리하며, 게임 데이터 서버나 데이터베이스 없이 정적 배포된다. API 키 보호용 LLM 프록시만 게임 본체와 분리되어 있다. 정적 호스팅의 고정 운영 비용은 사실상 0이고, LLM보다 먼저 조회하는 하드테이블 406건·IndexedDB 캐시·프리로드가 이미 알려진 조합과 반복 생성을 대부분 차단한다.

다음 단계는 현재 플레이를 바꾸지 않고 서비스 계층을 얹는 방식이다.
...
2. **최초 발견 기록** — 전역 캐시에 결과를 처음 쓴 익명 식별자를 함께 보관해 “이 개념을 처음 만든 사람”을 기록한다. 계정이나 로그인 없이도 가능한 확장이다.
