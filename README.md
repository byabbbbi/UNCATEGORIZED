...
데스크톱과 세로 모드 휴대폰(768px 미만)을 지원한다. 휴대폰에서는 하단 `공방`·`제단` 탭으로 생성과 선포를 오가고, `⋯` 메뉴에서 생성 규칙·연대기·시대 마감·정리를 연다.

## 조작

| 행동 | 실제 조작 |
| --- | --- |
| 조합 | 데스크톱에서는 캔버스 카드를 드래그해 겹친다. 휴대폰에서는 카드를 탭해 조합 슬롯 두 칸을 채우거나 그대로 드래그한다. 여러 조합을 동시에 처리할 수 있다. |
| 카드 복제 | 데스크톱에서는 더블클릭한다. 휴대폰에서는 카드를 400ms 길게 눌러 판례 시트를 연 뒤 `복제`를 누른다. |
| 판례 대장 | 데스크톱에서는 카드를 클릭한다. 휴대폰에서는 카드를 400ms 길게 눌러 부모 조합, 발견 당시 세계 상태, 파생 개념, 기둥·파괴력과 연대기를 시트로 연다. |
| 대장 · CODEX | 데스크톱에서는 `Tab`, 휴대폰에서는 하단 서랍의 `전체 N` 버튼으로 연다. 검색·정렬 후 개념을 클릭하면 캔버스에 다시 놓인다. |
| 선포 | 데스크톱에서는 우측 패널에서 대상을 고른 뒤 카드를 중앙 제단 `ARA`에 드래그한다. 휴대폰에서는 `제단` 탭에서 카드와 기둥을 차례로 고르고 선포 인장을 400ms 누른다. 시대당 최대 3회다. |
| 시대 마감 | 데스크톱에서는 캔버스 우측 상단, 휴대폰에서는 상단 `⋯` 메뉴의 `시대 마감`을 누른다. 사건을 정산하고 다음 시대로 넘어간다. |
| 분실물 보관소 | 하단의 보관소를 열고 파편 25개로 개념 하나를 회수하거나, 파편 250개로 판정불가 이상 1개 보장 10연 회수를 한다. |
...
| 붕괴된 세계에서 | 제4시대, 측정과 본질 기둥 붕괴, 「먼지」 오염 승격, 파편 25개인 고정 데모에서 시작한다. |
