...
UTF-16 단위 `slice(0,4)` 때문에 이모지 둘이 통과한 버그는 `Intl.Segmenter`로 첫 그래핌만 남겨 해결했다. 오염 세계에서 하드테이블 406건이 비활성화되던 조건은 붕괴 여부만 보도록 고치고 오염은 이름 후처리로 옮겼다. “벽돌부싯돌기류” 같은 concat 이름은 양쪽 `headNoun()` 포함 검사로 검출해 한 번 리롤한다.
...
출발점은 2026년 8월 10일 완성된 조합 엔진·4기둥·선포·붕괴·엔딩·보관소 기반([`afe1173`](https://github.com/byabbbbi/UNCATEGORIZED/commit/afe1173276aa6e8e20727eb85778502ed3042b89)). 챌린지 기간에는 사람이 기능별 완료 조건과 금지 범위를 먼저 제시했다. Codex는 저장소에서 영향 범위를 찾은 뒤 별도 브랜치의 구현·빌드·검증·브라우저 확인·커밋·PR 본문 작성을 담당했다.
...
PR #2~#5는 앞 작업을 기준 브랜치로 삼은 연속 작업이었다. 이후 사람이 병합 순서를 검토해 기능 커밋을 `main`에 통합했다. 실제 병합 커밋은 `9a4a855` → `78ef65b` → `70d837f` → `d4f2cba` → `f741406` 순서. 판례 대장·사건부는 문서 작성 시점에 PR #6으로 열려 있다.

### 2. 신규 기능

- **생성·저장 기반 정비**: few-shot 예시 3개와 부적절 이름 리롤 1회, 하드 조합 406건. 공백 제외 10자·최대 3어절 제한, 띄어쓰기 지원, 일반 런 저장·이어하기와 1·2시대 선포 배율도 코드로 고정했다.
...
- **이름·이모지 방어**: `headNoun()` 기반 concat 판정, 프롬프트 금지 규칙, 모든 결과 경로의 10자 제한을 통합했다. 이모지는 `Intl.Segmenter`로 첫 그래핌 하나만 남기고 CSS와 406건 검증 스크립트로 이중 방어.
...
기존 `emoji: String(r.emoji || '❔').slice(0, 4)`는 사용자가 보는 문자 수가 아니라 UTF-16 코드 단위를 잘랐다. 단순 이모지는 보통 두 코드 단위라 `🔥🗣️`, `💣📨`처럼 둘 이상이 통과할 수 있었고, ZWJ 가족 이모지나 피부색 변형도 중간에서 잘릴 수 있었다. [`0791b45`](https://github.com/byabbbbi/UNCATEGORIZED/commit/0791b4546a0ee9c072099c5f9a2f7a5098fbe5dd)에서 `Intl.Segmenter('ko', { granularity: 'grapheme' })`로 첫 그래핌만 취하고 미지원 환경은 첫 코드 포인트로 폴백했다. API 정규화뿐 아니라 하드테이블·폴백·가챠·일일 시작·데모·저장 복원에도 같은 유틸을 적용했다.
