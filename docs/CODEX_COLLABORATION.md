...
Codex는 챌린지 신규 기능의 요구사항 분해, 구현, 회귀 점검, 브라우저 확인, 커밋·PR 정리에 사용했다.

### 2. 어떤 기능을 구현했나

하드테이블 406건·리롤·띄어쓰기·저장, 오늘의 세계, 연대기 PNG, 카드 복제·조합명 검사, 그래핌 단위 이모지 제한, 8기둥, 판례 대장·사건부를 구현했다. 오늘의 세계는 1,000개 날짜에서 결정성과 후보 조건 오류 0건, 초기 붕괴 291건(29.1%)을 검증했다.
...
사람이 세계관과 밸런스 수식, 기능별 스펙, PR 검토·병합 전략, Worker 배포·API 키 관리·서버측 모델 정책, 플레이 테스트를 결정했다. Codex는 정해진 범위 안에서 구현하고 결과를 검증했다.
